    ...
    else
    {
        FOR_EACH_VEC_BACK(m_vecCurrentTrickAttempts, i)
        {
            ...
            if (pTrickAttempt->ShouldContinue(pZone, pPlayer))
            {
                ...
                {
                    const auto iCurrentStep = pTrickAttempt->GetCurrentStep();
...
void CTrickSystem::StartRecording()
{
    ...
    AssertMsg(false, "Needs to network to the client");
}
...
void CTrickSystem::TeleportToTrick(int iTrickID)
{
    ...
    CMomentumPlayer::GetLocalPlayer()->ManualTeleport(&pTrigger->WorldSpaceCenter(), nullptr, &vec3_origin);
}
...
const char* CTrickSystem::GetTricksFileName()
{
    return CFmtStr("%s/%s.tricks", ZONE_FOLDER, MapName());
}
